...
class RestCommon():
    ...
    def _assert_rest_result(self, result, err_str):
        ...
        if error_code == ERROR_CONNECT_TO_SERVER or error_code == ERROR_UNAUTHORIZED_TO_SERVER:
            LOG.error(_("can't open the recent url, relogin."))
            self.login()
    ...
    def delete_volume(self, volume):
        ...
        if lun_id:
            ...
            LOG.warning(_("Can't find lun or lungroup on the array."))
    ...
    def _check_lun_exist(self, lun_id):
        ...
        if error_code == ERROR_CONNECT_TO_SERVER or error_code == ERROR_UNAUTHORIZED_TO_SERVER:
            LOG.error(_("cant't open the recent url, relogin."))
            self.login()
            result = self.call(url, data, "GET")
    ...
    def _delete_lun(self, lun_id):
        lun_group_id = self._get_lungroupid_by_lunid(lun_id)
        if lun_group_id:
            self._remove_lun_from_lungroup(lun_group_id, lun_id)
    ...
    def _add_host_into_hostgroup(self, host_id):
        ...
        LOG.info(_('_add_host_into_hostgroup, host group name:%(name)s, '
                   'host group id:%(id)s.')
    ...
    def terminate_connection_fc(self, volume, connector):
        ...
        if lun_id:
            if self._check_lun_exist(lun_id) is True:
                ...
                lungroup_id = self._get_lungroupid_by_lunid(lun_id)
                if lungroup_id is None:
                    LOG.info(_("Can't find lun in lungroup."))
                else:
                    self._remove_lun_from_lungroup(lungroup_id, lun_id)
            else:
               LOG.warn(_("Can't find lun on the array"))
        ...
        if prefetch is not None and prefetch.attrib['Type']:
            if fetchtype in ['0', '1', '2', '3']:
                ...
                if lunsetinfo['PrefetchType'] == '1':
                    ...
                    typevalue_double = str(double_value)
