...
class TseriesCommon():
    ...
    def _get_maximum_capacity_pool_id(self, pools_conf, pools_dev, luntype):
        ...
        if luntype == 'Thick':
            ...
            while len(detailed_pools) > 0:
                # list.pop() will return the last item.
            ...
            try:
                ...
                if connect_times < 2:
                    ...
                    if self.ssh_pool and ssh_client:
                        self.ssh_pool.remove(ssh_client)
                    # set ssh_pool as None when connect error,
                    # or the next command connect will also error.
                    self.ssh_pool = None
    ...
    def extend_volume(self, volume, new_size):
        ...
        if lun_id is None:
            err_msg = (_('extend_volume: volume %s does not exist.')
                       % lun_id)
            LOG.error(err_msg)
            raise exception.VolumeNotFound(volume_id=lun_id)
        ...
        added_vol_size = (six.text_type(int(new_size) -
                         int(volume['size'])) + 'G')
        ...
        type_id = volume['volume_type_id']
        parameters = self._parse_volume_type(type_id)
        if ('LUNType' in parameters
                and parameters['LUNType'] == 'Thin'):
    ...
    def _update_volume_stats(self):
        ...
        LOG.debug('_update_volume_stats. vendor_name %(vendor_name)s,'
                  'total_capacity_gb %(total_capacity_gb)f,'
                  'free_capacity_gb %(free_capacity_gb)f,'
                  'reserved_percentage %(reserved_percentage)d,'
