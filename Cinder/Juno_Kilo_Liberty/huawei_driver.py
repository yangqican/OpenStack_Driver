...
class HuaweiV3ISCSIDriver(Huawei18000ISCSIDriver):
    """ISCSI driver for Huawei OceanStor V3 storage arrays.

    Version history:
        1.0.0 - Initial driver
        1.0.1 - Provide Huawei OceanStor V3 storage volume driver.
    """

    VERSION = "1.1.0"

    def __init__(self, *args, **kwargs):
        super(HuaweiV3ISCSIDriver, self).__init__(*args, **kwargs)
...
class HuaweiV3FCDriver(Huawei18000FCDriver):
    """FC driver for Huawei OceanStor V3 storage arrays.

    Version history:
        1.0.0 - Initial driver
        1.1.0 - Provide Huawei OceanStor V3 storage volume driver.
    """

    VERSION = "1.1.0"

    def __init__(self, *args, **kwargs):
        super(HuaweiV3FCDriver, self).__init__(*args, **kwargs)
    ...
    def initialize_connection(self, volume, connector):
        ...
        lun_id = (volume.get('provider_location', None) or
                  self.restclient.get_volume_by_name(volume_name))
        if not lun_id:
        ...
        if 'hypermetro_id' in metadata:
            try:
                ...
                hyperm.rmt_client.logout()
                msg = _("Attach hypermetro volume error: %s.") % err
                LOG.error(msg)
                raise exception.VolumeBackendAPIException(data=msg)
    ...
    def terminate_connection(self, volume, connector, **kwargs):
        ...
        if int(left_lunnum) > 0:
            ...
            fc_info = {'driver_volume_type': 'fibre_channel',
                       'data': {'target_wwn': tgt_port_wwns,
                                'initiator_target_map': init_targ_map}}
        LOG.info(_LI("terminate_connection_fc, return data is: %s."),
                 fc_info)
        ...
        if 'hypermetro_id' in metadata:
            try:
                ...
                LOG.error(_LE("Detach hypermetro error: %s."), err)

        return fc_info


class HuaweiTV2ISCSIDriver(Huawei18000ISCSIDriver):
    """ISCSI driver for Huawei OceanStor TV2 storage arrays.

    Version history:
        1.0.0 - Initial driver
        1.0.1 - Provide Huawei OceanStor TV2 storage volume driver.
    """

    VERSION = "1.1.0"

    def __init__(self, *args, **kwargs):
        super(HuaweiTV2ISCSIDriver, self).__init__(*args, **kwargs)
...
class HuaweiTV2FCDriver(Huawei18000FCDriver):
    """FC driver for Huawei OceanStor TV2 storage arrays.

    Version history:
        1.0.0 - Initial driver
        1.1.0 - Provide Huawei OceanStor TV2 storage volume driver.
    """

    VERSION = "1.1.0"

    def __init__(self, *args, **kwargs):
        super(HuaweiTV2FCDriver, self).__init__(*args, **kwargs)
